package com.jeecg.diaodufee.service.impl;
...
@Service("diaoduFeeService")
@Transactional
public class DiaoduFeeServiceImpl extends CommonServiceImpl implements DiaoduFeeServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(DiaoduFeeEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(DiaoduFeeEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(DiaoduFeeEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(DiaoduFeeEntity t){
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,DiaoduFeeEntity t){
 		...
 		sql  = sql.replace("#{famount1}",String.valueOf(t.getFamount1()));
 		sql  = sql.replace("#{fratedamount}",String.valueOf(t.getFratedamount()));
 		sql  = sql.replace("#{frealamount}",String.valueOf(t.getFrealamount()));
 		sql  = sql.replace("#{fcushion}",String.valueOf(t.getFcushion()));
 		sql  = sql.replace("#{fcheck}",String.valueOf(t.getFcheck()));
 		sql  = sql.replace("#{fattribute}",String.valueOf(t.getFattribute()));
 		...
 		sql  = sql.replace("#{fboxnum}",String.valueOf(t.getFboxnum()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("z_diaodu_fee",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
