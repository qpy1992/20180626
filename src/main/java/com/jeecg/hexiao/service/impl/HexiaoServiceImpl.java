package com.jeecg.hexiao.service.impl;
...
@Transactional
public class HexiaoServiceImpl extends CommonServiceImpl implements HexiaoServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(HexiaoEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(HexiaoEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(HexiaoEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(HexiaoEntity t){
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,HexiaoEntity t){
 		...
 		sql  = sql.replace("#{fbcamount}",String.valueOf(t.getFbcamount()));
 		sql  = sql.replace("#{factualamount}",String.valueOf(t.getFactualamount()));
 		sql  = sql.replace("#{famount}",String.valueOf(t.getFamount()));
 		sql  = sql.replace("#{ftrueamount}",String.valueOf(t.getFtrueamount()));
 		sql  = sql.replace("#{fdaidian}",String.valueOf(t.getFdaidian()));
 		sql  = sql.replace("#{fyuehao}",String.valueOf(t.getFyuehao()));
 		sql  = sql.replace("#{fnote}",String.valueOf(t.getFnote()));
 		sql  = sql.replace("#{ffapiao}",String.valueOf(t.getFfapiao()));
 		...
 		sql  = sql.replace("#{fpaiche}",String.valueOf(t.getFpaiche()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("z_hexiao",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
