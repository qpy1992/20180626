package com.jeecg.danger.service.impl;
...
@Transactional
public class DangerServiceImpl extends CommonServiceImpl implements DangerServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(DangerEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(DangerEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(DangerEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(DangerEntity t){
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,DangerEntity t){
 		...
 		sql  = sql.replace("#{fcardno}",String.valueOf(t.getFcardno()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("z_danger",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
