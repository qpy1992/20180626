package com.jeecg.trafficaccident.service.impl;
...
@Service("trafficAccidentService")
@Transactional
public class TrafficAccidentServiceImpl extends CommonServiceImpl implements TrafficAccidentServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(TrafficAccidentEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(TrafficAccidentEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(TrafficAccidentEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(TrafficAccidentEntity t){
		...
		map.put("fcxtype", t.getFcxtype());
		map.put("fname", t.getFname());
		map.put("frespon", t.getFrespon());
		map.put("fchuxian_time", t.getFchuxianTime());
		map.put("fsggrade", t.getFsggrade());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,TrafficAccidentEntity t){
 		...
 		sql  = sql.replace("#{fcxtype}",String.valueOf(t.getFcxtype()));
 		sql  = sql.replace("#{fname}",String.valueOf(t.getFname()));
 		sql  = sql.replace("#{frespon}",String.valueOf(t.getFrespon()));
 		sql  = sql.replace("#{fchuxian_time}",String.valueOf(t.getFchuxianTime()));
 		sql  = sql.replace("#{fsggrade}",String.valueOf(t.getFsggrade()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("z_traffic_accident",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
