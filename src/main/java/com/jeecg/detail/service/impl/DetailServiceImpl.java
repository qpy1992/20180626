package com.jeecg.detail.service.impl;
...
@Transactional
public class DetailServiceImpl extends CommonServiceImpl implements DetailServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(DetailEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(DetailEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(DetailEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(DetailEntity t){
		Map<String,Object> map = new HashMap<String,Object>();
		map.put("id", t.getId());
		map.put("fbillno", t.getFbillno());
		map.put("fname", t.getFname());
		map.put("fmaitou", t.getFmaitou());
		map.put("fpackage", t.getFpackage());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,DetailEntity t){
 		sql  = sql.replace("#{id}",String.valueOf(t.getId()));
 		sql  = sql.replace("#{fbillno}",String.valueOf(t.getFbillno()));
 		sql  = sql.replace("#{fname}",String.valueOf(t.getFname()));
 		sql  = sql.replace("#{fmaitou}",String.valueOf(t.getFmaitou()));
 		sql  = sql.replace("#{fpackage}",String.valueOf(t.getFpackage()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("z_detail",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
