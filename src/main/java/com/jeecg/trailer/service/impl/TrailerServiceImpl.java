package com.jeecg.trailer.service.impl;
...
@Transactional
public class TrailerServiceImpl extends CommonServiceImpl implements TrailerServiceI {
	@Autowired
	private SystemService systemService;
 	...
 	public Serializable save(TrailerEntity entity) throws Exception{
 		entity.setFcode(systemService.getFcode(37));
	 ...
	 * @return
	 */
	private void doAddBus(TrailerEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(TrailerEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(TrailerEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(TrailerEntity t){
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,TrailerEntity t){
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("z_trailer",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
