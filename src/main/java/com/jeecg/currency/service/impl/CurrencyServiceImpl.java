package com.jeecg.currency.service.impl;
...
public class CurrencyServiceImpl extends CommonServiceImpl implements CurrencyServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(CurrencyEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(CurrencyEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(CurrencyEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(CurrencyEntity t){
		...
		map.put("fcurrencyid", t.getFcurrencyid());
		map.put("fyouxiao", t.getFyouxiao());
		map.put("fbenwei", t.getFbenwei());
		map.put("fname", t.getFname());
		map.put("fsymbol", t.getFsymbol());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,CurrencyEntity t){
 		...
 		sql  = sql.replace("#{fcurrencyid}",String.valueOf(t.getFcurrencyid()));
 		sql  = sql.replace("#{fyouxiao}",String.valueOf(t.getFyouxiao()));
 		sql  = sql.replace("#{fbenwei}",String.valueOf(t.getFbenwei()));
 		sql  = sql.replace("#{fname}",String.valueOf(t.getFname()));
 		sql  = sql.replace("#{fsymbol}",String.valueOf(t.getFsymbol()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("z_currency",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
