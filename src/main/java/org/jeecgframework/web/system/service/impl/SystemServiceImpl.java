...
//		log.setTSUser(ResourceUtil.getSessionUser());
     ...
     * @return
     */
    public String getFcode(int fbilltype){
        String FProjectVal1 = jdbcTemplate.queryForObject("select FProjectVal from t_billCodeRule where FBillType=? and FProjectID=1",String.class,fbilltype);
        String FProjectVal2 = jdbcTemplate.queryForObject("select FProjectVal from t_billCodeRule where FbillType=? and FProjectID=3",String.class,fbilltype);
        String suffix = "000"+FProjectVal2;
        String fcode = FProjectVal1+suffix.substring(suffix.length()-4,suffix.length());
        return fcode;
    }
	 ...
	 * @return
	 */
	public BigDecimal removePoint(String num){
		return new BigDecimal(Double.valueOf(num).intValue());
	}
	 ...
	 * @return
	 */
	public Date getnDay(int num,Date date) {
