...
public class SystemController extends BaseController {
	...
	@RequestMapping(params = "typeGroupList1")
	public ModelAndView typeGroupList1(HttpServletRequest request) {
		return new ModelAndView("system/type/typeGroupList1");
	}
	...
	@RequestMapping(params = "typeGroupGrid1")
	public void typeGroupGrid1(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid, TSTypegroup typegroup) {
		CriteriaQuery cq = new CriteriaQuery(TSTypegroup.class, dataGrid);
		//查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, typegroup, request.getParameterMap());
		try{
			//自定义追加查询条件
			cq.eq("typegroupid",1);
		}catch (Exception e) {
...
//    }
	...
	@RequestMapping(params = "aouTypeGroup1")
	public ModelAndView aouTypeGroup1(TSTypegroup typegroup, HttpServletRequest req) {
		if (typegroup.getId() != null) {
			typegroup = systemService.getEntity(TSTypegroup.class, typegroup.getId());
			req.setAttribute("typegroup", typegroup);
		}
		return new ModelAndView("system/type/typegroup1");
	}
